...
def fit(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for img, y, w in tqdm(train_loader, disable=True):
            img = img.cuda()
            y = y.cuda()
            w = w.cuda()

            if np.random.random() < data_config["mix_proba"]:
                img, y, _, w = mix(img, y, w=w, use_3d=True)

            with torch.cuda.amp.autocast(enabled=use_fp16):
                y_pred, y_pred_aux = model(img)

                loss = loss_fct(y_pred, torch.zeros(1), y, torch.zeros(1))  # , w=w
